...
export function ProductCard({ tile }: { tile: ProductTile }) {
  return (
    <motion.div whileHover={{ y: -6 }} transition={{ duration: 0.25 }} className="group rounded-3xl border border-white/10 bg-panel p-4">
      <Link to={tile.href} className="block space-y-4">
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10">
          <img src={tile.image} alt={tile.title} className="h-40 w-full object-cover transition duration-300 group-hover:scale-105" />
        </div>
        <div>
